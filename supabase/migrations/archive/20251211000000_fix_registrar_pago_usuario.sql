-- 🤖 FUNCIÓN MAESTRA: Procesamiento de Pagos v3.1 (Hotfix Usuario ID)
-- Maneja Renovaciones (Interés) y Desempeños (Capital + Interés)
-- CHANGELOG: Se agrega p_usuario_id para cumplir constraint NOT NULL en movimientos_caja_operativa
...
CREATE OR REPLACE FUNCTION public.registrar_pago_oficial(
    ...
    p_usuario_id UUID DEFAULT auth.uid()  -- Usuario que realiza la operación (Nuevo param)
)
...
BEGIN
    ...
    INSERT INTO public.pagos (
        ...
        usuario_id -- Se agrega usuario_id también aquí si la tabla lo tiene
    ) VALUES (
