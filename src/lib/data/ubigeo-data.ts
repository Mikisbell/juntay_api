...
export const UBIGEO_DATA: UbigeoNode[] = [
  {
    "id": "01",
    "nombre": "Amazonas",
    "hijos": [
...
export const getDepartamentos = () => UBIGEO_DATA;

export const getProvincias = (deptId: string) => {
  const dept = UBIGEO_DATA.find(d => d.id === deptId);
  return dept ? dept.hijos || [] : [];
};

export const getDistritos = (provId: string) => {
  for (const dept of UBIGEO_DATA) {
    if (dept.hijos) {
      const prov = dept.hijos.find(p => p.id === provId);
      if (prov) {
        return prov.hijos || [];
      }
    }
