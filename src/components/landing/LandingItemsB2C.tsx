...
export function LandingItemsB2C() {
    return (
        <section className="py-16 bg-white" id="assets">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {items.map((item, index) => (
                        <motion.div
                            ...
                            className="bg-gray-50 hover:bg-[#D4AF37]/10 border border-gray-100 hover:border-[#D4AF37]/30 rounded-2xl p-6 text-center transition-all group cursor-default"
                        >
                            <div className="w-14 h-14 mx-auto mb-4 bg-white rounded-xl flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:bg-[#1E3A5F]/10 transition-all">
                                <item.icon className="w-7 h-7 text-[#1E3A5F] group-hover:text-[#D4AF37] transition-colors" />
                            </div>
