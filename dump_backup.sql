...
COMMENT ON FUNCTION "public"."admin_asignar_caja"("p_usuario_cajero_id" "uuid", "p_monto" numeric, "p_observacion" "text") IS 'Asigna fondos a un cajero seleccionando automáticamente la bóveda con mayor liquidez.';
...
CREATE OR REPLACE FUNCTION "public"."check_client_status_before_credit"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION "public"."fn_liquidar_caja_cerrada"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION "public"."fn_protect_ledger_integrity"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."fn_update_caja_operativa_saldo"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    factor NUMERIC := 1;
...
CREATE OR REPLACE FUNCTION "public"."fn_update_cuentas_financieras_saldo"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
CREATE TABLE IF NOT EXISTS "public"."transacciones_capital" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
CREATE OR REPLACE TRIGGER "trg_auto_liquidar_caja" AFTER UPDATE ON "public"."cajas_operativas" FOR EACH ROW WHEN ((("old"."estado")::"text" IS DISTINCT FROM ("new"."estado")::"text")) EXECUTE FUNCTION "public"."fn_liquidar_caja_cerrada"();
...
CREATE OR REPLACE TRIGGER "trg_ledger_smart_lock" BEFORE DELETE OR UPDATE ON "public"."transacciones_capital" FOR EACH ROW EXECUTE FUNCTION "public"."fn_protect_ledger_integrity"();



CREATE OR REPLACE TRIGGER "trg_movimiento_caja_update_saldo" AFTER INSERT ON "public"."movimientos_caja_operativa" FOR EACH ROW EXECUTE FUNCTION "public"."fn_update_caja_operativa_saldo"();
...
CREATE OR REPLACE TRIGGER "trg_security_validate_client_active" BEFORE INSERT ON "public"."creditos" FOR EACH ROW EXECUTE FUNCTION "public"."check_client_status_before_credit"();
...
CREATE OR REPLACE TRIGGER "trg_transaccion_capital_update_saldo" AFTER INSERT ON "public"."transacciones_capital" FOR EACH ROW EXECUTE FUNCTION "public"."fn_update_cuentas_financieras_saldo"();
...
GRANT SELECT,INSERT,REFERENCES,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."pagos" TO "anon";
GRANT SELECT,INSERT,REFERENCES,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."pagos" TO "authenticated";
